...
public class AddTransactionActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    addTransactionBtn.setOnClickListener(
        v -> {
          ...
          if (isGroupTransactionCheckBox.isChecked()
              && !verifyGroupShares(Double.parseDouble(amount))) {
            View parentLayout = findViewById(android.R.id.content);
            Snackbar snackbar =
                Snackbar.make(
                    parentLayout,
                    "Share amounts should add up to Transaction amount!",
                    Snackbar.LENGTH_LONG);
            View sbView = snackbar.getView();
            sbView.setBackgroundColor(
                ContextCompat.getColor(AddTransactionActivity.this, R.color.accentGreen));
            snackbar.show();
            return;
          }
    ...
    isGroupTransactionCheckBox.setOnClickListener(
        v -> {
          if (isGroupTransactionCheckBox.isChecked()) {
    ...
    addFriendToShareBtn.setOnClickListener(
        v -> {
          addNewFriendToShareLayout();
        });
  }
  ...
  public void addNewFriendToShareLayout() {
    View addView =
        layoutInflater.inflate(R.layout.group_transaction_share, layoutOfUserShares, false);
    layoutOfUserShares.addView(addView);
    ImageButton removeShareBtn = addView.findViewById(R.id.removeShareBtn);
    removeShareBtn.setOnClickListener(
        v -> {
